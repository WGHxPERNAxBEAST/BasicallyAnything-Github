...
public class BlockTinFenceGate extends BlockHorizontal {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        worldIn.playSound(playerIn, pos, ((Boolean)state.getValue(OPEN)).booleanValue() ? BatSoundHandler.TIN_FENCE_GATE_OPEN : BatSoundHandler.TIN_FENCE_GATE_CLOSE, SoundCategory.BLOCKS, 1.0F, 1.0F);
        ...
        if (!worldIn.isRemote)
        {
            ...
            if (((Boolean)state.getValue(POWERED)).booleanValue() != flag)
            {
                ...
                if (((Boolean)state.getValue(OPEN)).booleanValue() != flag)
                {
                	...
                	worldIn.playSound(null, pos, flag ? BatSoundHandler.TIN_FENCE_GATE_OPEN : BatSoundHandler.TIN_FENCE_GATE_CLOSE, SoundCategory.BLOCKS, 1.0F, 1.0F);
                }
            }
        }
    }
